...
**Pipeline is automated** via the "Phase 2a Python — Weekly Content" Claude Routine (Tuesday 11:00 WAT). Configure at `claude.ai/code/routines`.

**PR review commands** (comment on the PR):
- `/approve` → handled by `pr-commands.yml` (squash merge + delete branch)
- `/rework [feedback]` → handled by the "Phase 2a Python — Content Rework" Claude Routine
- `/reject [reason]` → handled by `pr-commands.yml` (close + delete branch + Telegram)
