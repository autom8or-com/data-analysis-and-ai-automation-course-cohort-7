...
- **Join fan-out lint** — a static `%%sql` scan (all notebook types) that raises a `fanout_risk` failure when a query directly joins **two or more** of the many-per-order fact tables (`order_items`, `order_payments`, `order_reviews`) and then aggregates (`SUM`/`AVG`/non-DISTINCT `COUNT`). This is the Phase 2a Week-7 defect class — the reviews join fanned out rows and inflated category revenue. SQL comment lines (`--`) are stripped first, so the §6 "common mistake" comment block never trips it; a query that pre-aggregates in a `WITH` CTE clears it. Fix: collapse the non-base tables to one row per `order_id` in a CTE before joining, and use `COUNT(DISTINCT order_id)`.
- **Env-driven execution** — for Week 3+ demo/solutions, the script runs `jupyter nbconvert --execute` with `OLIST_DATA_PATH` (the extracted Olist CSV folder) and `OLIST_DB_PATH` (a fresh tempfile `.db`) exported into the kernel env. The portable setup cell reads both at runtime and, off Colab, skips `drive.mount` via its `try/except ModuleNotFoundError`, so **no source rewriting is required**. (The script still applies the legacy `/content` string replacements as a harmless no-op fallback for any older notebook that still carries the hardcoded setup cell.)
